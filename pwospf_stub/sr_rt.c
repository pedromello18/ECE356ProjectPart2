...
void *sr_rip_timeout(void *sr_ptr) {
    ...
    {  
        ...
        while (cur_rt) {
            struct sr_rt *internal_rt = cur_rt->next;
            int flag = 0;
            while (internal_rt) {
                if (internal_rt->dest.s_addr == cur_rt->dest.s_addr) {
                    flag = 1;
                    break;
                }
                internal_rt = internal_rt->next;
            }
            if ((difftime(now, cur_rt->updated_time) > 20) || (sr_obtain_interface_status(sr, cur_rt->interface) == 0) || ((cur_rt->dest.s_addr & cur_rt->mask.s_addr) == (cur_rt->gw.s_addr & cur_rt->mask.s_addr)) || flag)
            {
...
void send_rip_update(struct sr_instance *sr){
    ...
    while (cur_if) {
        if (sr_obtain_interface_status(sr, cur_if->name) == 1){
            ...
            uint8_t *p_packet = (uint8_t *)calloc(1, sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t) + sizeof(sr_rip_pkt_t) + sizeof(sr_udp_hdr_t));
            sr_ethernet_hdr_t *p_ethernet_header = (sr_ethernet_hdr_t *)p_packet;
            sr_ip_hdr_t *p_ip_header = (sr_ip_hdr_t *)((p_packet + sizeof(sr_ethernet_hdr_t)));
            sr_udp_hdr_t *p_udp_header = (sr_udp_hdr_t *)(p_packet + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
            sr_rip_pkt_t *p_rip_packet = (sr_rip_pkt_t *)(p_packet + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t) + sizeof(sr_udp_hdr_t));
            ...
            while (routing_entry && (entry_index < MAX_NUM_ENTRIES))
            {
                ...
                if((cur_if->ip & routing_entry->mask.s_addr) == (routing_entry->gw.s_addr * routing_entry->mask.s_addr))
                {
        ...
        while(cur_rt && (! entry_found))
        {
            if (cur_rt->dest.s_addr == rip_packet->entries[i].address) {
                ...
                if (rip_packet->entries[i].metric < INFINITY) {
                    cur_rt->updated_time = time(NULL);
                    printf("updated time from arriving entry to %i with metric %i", rip_packet->entries[i].address, rip_packet->entries[i].metric);
                }
